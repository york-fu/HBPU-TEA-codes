...
{
	static double pid_value, pwmvalue;
	
	if(TIM_GetFlagStatus(TIM5, TIM_IT_Update)!=RESET)	//判断中断为更新中断,10ms
	{
		pid_value += PID_Adjust(dis);	//PID获取本次值
		
		if(pid_value>600)  pid_value = 600;		//输出限幅
		if(pid_value<-300) pid_value = -300;
		
		pwmvalue = pid_value + 5760;
		TIM_SetCompare1(TIM3,pwmvalue);
		LCD_ShowxNum(92,100,pwmvalue,5,16,0x80);
