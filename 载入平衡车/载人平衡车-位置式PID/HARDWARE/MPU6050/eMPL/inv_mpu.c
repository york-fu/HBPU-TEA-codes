...
short Gyro[2][3];
u8 mpu_dmp_get_data(float *roll)
{
	float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
	short sensors, accel[2][3];
	unsigned long sensor_timestamp;
	unsigned char more;
	long quat[4]; 
			
	if(dmp_read_fifo(Gyro[USE_MPU-1], accel[USE_MPU-1], quat, &sensor_timestamp, &sensors,&more))return 1;
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		/*********pitch角范围-180~180************/
		roll[USE_MPU-1] = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;   // roll  
//		if(fabs(roll[USE_MPU-1]) < 90.0)    pitch[USE_MPU-1] = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;    
//		else     pitch[USE_MPU-1] = (fabs(q0* q2 - q1 * q3)/(q0* q2 - q1 * q3)) *(180.0 - fabs(asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3));
		
//		*pitch = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;	// pitch范围 -90 ~ 90
