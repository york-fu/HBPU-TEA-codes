...
void SIMUL_SPI_Init(void)  
{
	...
	RCC_APB2PeriphClockCmd(RCC_SCLK|RCC_MOSI|RCC_MISO|RCC_APB2Periph_AFIO, ENABLE);//时钟使能
	
	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//关闭jtag，使能SWD，可以用SWD模式调试

	GPIO_InitStructure.GPIO_Pin =  GPIO_SCLK_PIN;				//SCK引脚
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 			//速度
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 			//推挽输出
	GPIO_Init(GPIO_SCLK_PORT, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin =  GPIO_MOSI_PIN;				//MOSI引脚
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; 			//速度
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 			//推挽输出
	GPIO_Init(GPIO_MOSI_PORT, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = GPIO_MISO_PIN;        		//MISO输入管脚
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;      		//速度
