...
const u8 RX_ADDRESS[RX_ADR_WIDTH]={0x23,0x23,0x32,0x32,0x01}; //接收地址
...
{ 	
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;			//PG6输入  
	GPIO_Init(GPIOG, &GPIO_InitStructure);

	GPIO_ResetBits(GPIOG,GPIO_Pin_7|GPIO_Pin_8);	//PG7,8上拉					 
		 
	SPI2_Init();    		//初始化SPI	
//	SPI2_SetSpeed(SPI_BaudRatePrescaler_8); //spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）
	
	NRF24L01_CE=0; 			//使能24L01
	NRF24L01_CSN_H;			//SPI取消片选
...
//	NRF24L01_Write_Reg(NRF_WRITE_REG+CONFIG, 0x0f);//配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式
	NRF24L01_Write_Reg(NRF_WRITE_REG+CONFIG, 0x3f);//配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式,开启接收中断 
