...
u8 OV7670_Init(void)
{
	...
	//设置IO
 	...
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA | RCC_APB2Periph_GPIOC, ENABLE);	 //使能相关端口时钟
	...
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_3;			//输入上拉
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
 	GPIO_Init(GPIOA, &GPIO_InitStructure);
	...
	GPIO_SetBits(GPIOC,GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_15);
