...
{  							  
	...
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数	
 	if(size!=20)num=num-' ';//得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字库）
	for(t=0;t<csize;t++)
	{   
...
{  							  
  u8 temp,t1,t;
	...
	for(t=0;t<csize;t++)
	{   
		if(size==16)temp=chinese_1616[num][t]; 	 	//调用1616字体
		else if(size==24)temp=chinese_2424[num][t];	//调用2424字体
		else if(size==30)temp=chinese_3030[num][t];	//调用3030字体
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowStrings(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p)
{         
	u8 x0=x;
	width+=x;
	height+=y;
