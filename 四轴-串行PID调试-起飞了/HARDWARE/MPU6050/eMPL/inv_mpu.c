...
u8 mpu_dmp_get_data(SITE *angle, SITE *palstance)
{
	...
	unsigned char more;
	long quat[4]; 
	static char flag=0;  // 该标志为用来标记读取数据次数
	static SITE AVGangle={0, 0, 0};  // 用来取平均值 
	static SITE AVGgyro={0, 0, 0};  // 用来取平均值 
	...
	**/
	if(sensors&INV_WXYZ_QUAT) 
	{
		...
		AVGangle.y += roll;
		AVGangle.z += yaw;
		AVGgyro.x  += gyro[1];  // 角速度
		AVGgyro.y  += gyro[0];
		AVGgyro.z  += gyro[2];
		flag++;
		if(flag==2)  // 取两次平均值
		{
			angle->x = AVGangle.x / 2;  // 角度  
			angle->y = AVGangle.y / 2;
			angle->z = AVGangle.z / 2;
			AVGangle.x = 0;
			AVGangle.y = 0;
			AVGangle.z = 0;
			palstance->x = AVGgyro.x / 2;  // 角速度
			palstance->y = AVGgyro.y / 2;
			palstance->z = AVGgyro.z / 2;
			AVGgyro.x = 0;
			AVGgyro.y = 0;
			AVGgyro.z = 0;
			flag=0;
