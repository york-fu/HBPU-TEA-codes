...
void Interface_Init()
{
	...
	LCD_ShowString(100,110,400,24,24,(u8*)"Expect_Angle: ");
	...
	if(Press_Flag)
	{
		...
		{	
			if(AimAngleSet)		//设置角度
			{
				...
				LCD_Fill(268,110,304,134,LIGHTBLUE);
				LCD_ShowxNum(268,110,SetAngle,3,24,0x80);
				BACK_COLOR = BLACK;
			}
			else				//未设置
			{			
				BACK_COLOR = LIGHTBLUE;
				LCD_Fill(268,110,304,134,LIGHTBLUE);
				LCD_ShowxNum(268,110,(int)AimAngle,3,24,0x80);
				BACK_COLOR = BLACK;
			}	
		}
		else
		{
			LCD_Fill(268,110,304,134,BLACK);
			LCD_ShowxNum(268,110,(int)AimAngle,3,24,0x80);
		}
