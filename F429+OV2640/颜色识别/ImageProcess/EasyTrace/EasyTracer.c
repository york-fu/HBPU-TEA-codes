...
#define HIGHT_MIN 100
#define HIGHT_MAX 300

#define StepSize 1

/*结果判别阀值*/
#define Threshold (((WIDTH_MAX-WIDTH_MIN)/StepSize)*((HIGHT_MAX-HIGHT_MIN)/StepSize))*0.6
...
void ColorDiscern(void)
{
	...
	/*绘制识别区域*/
	POINT_COLOR=RED;		//红色
	LCD_DrawRectangle(WIDTH_MIN,HIGHT_MIN,WIDTH_MAX,HIGHT_MAX);
	POINT_COLOR=WHITE;		//白色
		
	for(y=HIGHT_MIN;y<HIGHT_MAX;y+=StepSize)  // 处理区域中数据
	{
		for(x=WIDTH_MIN;x<WIDTH_MAX;x+=StepSize)
		{
